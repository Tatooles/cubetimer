...
function drawChart() {
  ...
  const series = progressSeries(props.solves).filter((point) => point.single != null);
...
onMounted(drawChart);
watch(() => props.solves, drawChart, { deep: true });
</script>

<template>
  <canvas ref="canvasRef" class="h-[150px] w-full" />
</template>
