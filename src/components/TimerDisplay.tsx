...
export function TimerDisplay({ elapsed, onKeyDown, scramble, scrambleActions, statusText, timerState }: TimerDisplayProps) {
  return (
    <div className={timerDisplayClass(timerState)} tabIndex={0} onKeyDown={onKeyDown}>
      ...
      <div className="text-center text-[0.96rem] leading-[1.2] font-[650] text-[#8d99aa] max-[680px]:text-[0.86rem]">{statusText}</div>
    </div>
  );
}
...
function timerDisplayClass(timerState: TimerState): string {
  return classNames(
    "grid min-h-0 grid-rows-[minmax(72px,auto)_minmax(120px,1fr)_auto] items-center rounded-lg border p-[18px_22px_24px] outline-none max-[680px]:grid-rows-[minmax(48px,auto)_minmax(70px,1fr)_auto] max-[680px]:p-3.5",
