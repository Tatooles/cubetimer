...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {} }) {
  ...
  // Self-heal: a saved (or user-driven) pan/zoom can park the viewport in
  // empty space with every artboard off-screen — the canvas then looks blank
  // / broken with no obvious way back. After mount, if nothing is visible,
  // frame all the content. Runs once layout has settled.
  React.useEffect(() => {
    ...
    const fitToContent = () => {
      ...
      slots.forEach((s) => {
        ...
        // invert current transform: screen → world
        minX = Math.min(minX, (r.left - vr.left - t.x) / t.scale);
        minY = Math.min(minY, (r.top - vr.top - t.y) / t.scale);
        maxX = Math.max(maxX, (r.right - vr.left - t.x) / t.scale);
        maxY = Math.max(maxY, (r.bottom - vr.top - t.y) / t.scale);
      });
      const cw = maxX - minX, ch = maxY - minY;
      if (!(cw > 0) || !(ch > 0)) return;
      const padX = 80, padTop = 72;
      const scale = Math.min(maxScale, Math.max(minScale,
        Math.min((vr.width - padX * 2) / cw, (vr.height - padTop - 80) / ch)));
      t.scale = scale;
      t.x = (vr.width - cw * scale) / 2 - minX * scale;
      t.y = padTop - minY * scale;
      apply();
    };
    ...
    const isStranded = () => {
      ...
      for (const s of slots) {
        const r = s.getBoundingClientRect();
        if (r.right > vr.left + 24 && r.left < vr.right - 24 &&
            r.bottom > vr.top + 24 && r.top < vr.bottom - 24) return false;
      }
      return true;
    };

    // Children mount asynchronously (after a sidecar read that can take a
    // while), so wait for the artboards to appear — via observer plus a
    // polling fallback — then run the stranded-check exactly once.
    let done = false, timer = 0, deadline = Date.now() + 8000;
    const check = () => {
      if (done) return;
      if (world.querySelector('[data-dc-slot]')) {
        done = true;
        if (isStranded()) fitToContent();
        cleanup();
        return;
      }
      if (Date.now() < deadline) timer = setTimeout(check, 120);
      else cleanup();
    };
    const mo = new MutationObserver(() => check());
    mo.observe(world, { childList: true, subtree: true });
    function cleanup() { clearTimeout(timer); mo.disconnect(); }
